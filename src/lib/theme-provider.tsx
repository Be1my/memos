...
const UserThemeSchema = z
	.enum(["light", "dark", "paper", "system"])
	.catch("system");
const AppThemeSchema = z.enum(["light", "dark", "paper"]).catch("light");
...
const themeScript = (() => {
	function themeFn() {
		try {
			...
			const validTheme = ["light", "dark", "paper", "system"].includes(
				storedTheme,
			)
				? storedTheme
				: "system";
